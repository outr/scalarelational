  ...
  "InheritanceSpec" should {
    ...
    "query content with mapper" in {
      withSession { implicit session =>
        ...
        query.to[InheritanceDatabase.Content](Content).converted.head should equal (Content("content2", Some(2)))
      }
    }
  }
}

object InheritanceDatabase extends H2Database(mode = H2Memory("inheritance_test")) {
