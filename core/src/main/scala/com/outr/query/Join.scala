 ...
 */
case class Join(table: Table, joinType: JoinType = JoinType.Join, condition: Condition)

// Used for DSL before the actual Join instance is created
case class PartialJoin(query: Query, table: Table, joinType: JoinType) {
  def on(condition: Condition) = query.copy(joins = (Join(table, joinType, condition) :: query.joins.reverse).reverse)
}
