...
case class SelectQueryPart[Types, Result](expressions: SelectExpressions[Types],
                                          ...
                                          distinct: Boolean) {
  def from(table: Table): Query[Types, Result] = {
    Query[Types, Result](expressions, table.datastore, Some(table), converter = converter, distinct = distinct)
  }
  ...
  def query(implicit session: Session): Query[Types, Result] =
    Query[Types, Result](expressions, session.datastore, None, converter = converter, distinct = distinct)
}