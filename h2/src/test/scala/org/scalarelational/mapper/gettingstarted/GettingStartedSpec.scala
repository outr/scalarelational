  ...
  "H2 examples" should {
    ...
    "Insert some Suppliers" in {
      ...
      session {
        ...
        acmeId = insert(suppliers, name("Acme, Inc."), street("99 Market Street"), city("Groundsville"), state(Some("CA")), status(Status.Unconfirmed), zip("95199")).result
        superiorCoffeeId = insert(suppliers, name("Superior Coffee"), street("1 Party Place"), city("Mendocino"), zip("95460"), status(Status.Unconfirmed)).result
    ...
    "Query all Coffees filtering and joining with Suppliers" in {
      session {
        val query = select(coffees.name, suppliers.name) from coffees innerJoin suppliers on coffees.supID === suppliers.ref where coffees.price < 9.0
